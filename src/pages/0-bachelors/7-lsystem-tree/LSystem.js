...
lSystem.LSystem.prototype.generate = function (generationCount) {
  ...
  var regStr = [];
  for (const eachVar in this.rules) regStr.push(eachVar);
  var regExp = new RegExp(regStr.join("|"), "g");
  // Apply rules for given generation count untill the end of civilization
  for (var i = 0; i < generationCount; i++) {
    currentString = currentString.replace(regExp, (stateVar) => {
      return this.rules[stateVar];
    });
  }
...
lSystem.LRunHandler.prototype = {
  on: function (stateVar, handler) {
    if (typeof handler == "function") this.stateFuncs[stateVar] = handler;
  },
  exec: function (stateVar) {
    var oldState = lSystem.util.oClone(this.stateVars);
    var currStateFunc = this.stateFuncs[stateVar];
    /**
